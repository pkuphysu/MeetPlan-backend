...
class OpenIDClientBackend:
    ...
    def get_token(client, code):
        res = requests.post(client.token_endpoint,
                            data={
                                'code': code,
    ...
    def get_userinfo(client, token):
        res = requests.get(client.userinfo_endpoint,
                           headers={
    ...
    def authenticate(self, request, code, **kwargs):
        ...
        if created1:
            ...
            user.save(update_fields=['name', 'email', 'website', 'phone_number', 'address', 'is_teacher', 'introduce'])
